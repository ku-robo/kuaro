...
U_WAY::U_WAY(){
	...
	uw_pt.getParam("click_width", click_width);
	uw_pt.getParam("resolution", resolution);
	uw_pt.getParam("output_file_name", file_name);
	uw_pt.getParam("load_file_name", load_file_name);
...
U_WAY::~U_WAY(){
	std::ofstream ofs;
	ofs.open(file_name.c_str() , std::ios::out);//ファイルを新しくではなく追記するばあい
	if (ofs.fail()){
