...
void C_person_manager::get_person_callback(const geometry_msgs::PointStamped::ConstPtr& sub_point)
{
	...
	if(person_point.size() > 0)
	{
		...
		{
			double person_diff = hypot(person_point[i].position[0] - sub_point->point.x, person_point[i].position[1] - sub_point->point.y);
	...
	person_flag = true; // fake_obstacle need comment out
}
