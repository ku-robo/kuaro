...
int main(int argc,char *argv[])
{
	...
	for(int i = 0; i < goal_point.size(); i++) //i = 18 person
	{
		...
		while(ros::ok())
		{
			...
			if(person_flag == false)
			{
				...
				else if (ac.getState() == actionlib::SimpleClientGoalState::ABORTED)
				{
					...
					while (ros::ok())
					{
						...
						std::cout << "back now !! " << "," << state.toString() << std::endl;
						...
						cmd_pub.publish(cmd_vel); //2017_10_18_kimoto
					...
					state = ac.getState();
					std::cout << "back finiiiiiish!! " << state.toString() << std::endl;
				}
			...
			//std::cout << "looooop!! " << state.toString() << std::endl;
			ros::spinOnce();
		}
	}
