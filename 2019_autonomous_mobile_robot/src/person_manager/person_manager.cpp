...
C_person_manager::C_person_manager(void)
{
	...
	ph.param("fileName", fileName, std::string("/home/robo/last_waypoint.txt"));
	...
	if(map_flag == 1)
	{
		...
		min_obstacle_setter.push_back(std::make_pair(64, 0.3));
		min_obstacle_setter.push_back(std::make_pair(73, 0.8));
		min_obstacle_setter.push_back(std::make_pair(257, 0.5));
		min_obstacle_setter.push_back(std::make_pair(299, 0.1));
		min_obstacle_setter.push_back(std::make_pair(309, 0.5));
		min_obstacle_setter.push_back(std::make_pair(346, 0.8));
		min_obstacle_setter.push_back(std::make_pair(492, 0.3));
		min_obstacle_setter.push_back(std::make_pair(522, 0.8));

		// min_obstacle_setter.push_back(std::make_pair(0, 0.8));
		// min_obstacle_setter.push_back(std::make_pair(41, 0.5));
...
void C_person_manager::get_person_callback(const geometry_msgs::PointStamped::ConstPtr& sub_point)
{
	...
	if(person_flag == true)
	{
		std::cout << "navigation to person now!!" << std::endl;
		return;
	}

	// 探索エリアなら人物までナビゲーションを行う
	if((130 < goal_index && goal_index < 230) || (373 < goal_index && goal_index < 406))
	{
		//何もしない
	}
	else
	{
		std::cout << "Target is Out of the area" << std::endl;
		return;
	}

	person_time = sub_point->header.stamp;

	// velodyneから得た人物の座標を map座標系に変換する
	geometry_msgs::PoseStamped gm_base_person;
	geometry_msgs::PoseStamped gm_map_person;
	gm_base_person.header.frame_id = "/base_link";
	gm_base_person.header.stamp = person_time;
	gm_base_person.pose.position.x = sub_point->point.x;
	gm_base_person.pose.position.y = sub_point->point.y;
	gm_base_person.pose.position.z = 0.0;
	gm_base_person.pose.orientation.x = 0.0;
	gm_base_person.pose.orientation.y = 0.0;
	gm_base_person.pose.orientation.z = 0.0;
	gm_base_person.pose.orientation.w = 1.0;
	gm_map_person = get_gm(gm_base_person, "/base_link", "/map", person_time);

	// 一度検出した人物が検出済みのものか判断する
	bool person_diff_flag = false;
	if(person_point.size() > 0)
	{
		for(int i=0; i< person_point.size(); ++i)
		{
			double person_diff = hypot(person_point[i].position[0] - gm_map_person.pose.position.x, person_point[i].position[1] - gm_map_person.pose.position.y);
	...
	if(person_point.size() > 4)
	{
		std::cout << "I will not detect target anymore" << std::endl;
		return;
	}

	std::cout << "Search Target in area!" << std::endl;

	// サブスクライバのデータを受け取る．　z軸の座標はヨー角を入れている（一応）
	now_person.position[0] = gm_map_person.pose.position.x;
	now_person.position[1] = gm_map_person.pose.position.y;
	now_person.position[2] = 0.0;
...
void C_person_manager::min_obstacle_set(int goal_index, int abort_count)
{
	std::string tmp_str;
	...
	for(int i = 0; i < set_num-1; ++i)
	{
		...
		{
			// abort_countが奇数なら min_obstacle_dist を 0.1 に設定する
			if(abort_count%2 == 1)
			{
				now_min_obstacle_value = 0.1;
				std::string tmp_str = min_obs_str + std::to_string(0.1);
			...
			// abort_countが偶数で設定値が前回と異なるなら， min_obstacle_dist を設定し直す
...
void C_person_manager::run()
{
	...
	for(int i = start_point; i < (int)(goal_point.size()); i++)
	{
		...
		min_obstacle_set(goal_index, abort_count);
		...
		std::stringstream ss;
		ss << i << " " << goal_point[i].position[0] << ", " << goal_point[i].position[1];
		ofs << ss.str() << std::endl;
		ac.sendGoal(goal);

		while(ros::ok())
		{
			// http://docs.ros.org/diamondback/api/actionlib/html/classactionlib_1_1SimpleClientGoalState.html
			...
			else
			{
				...
				person_start_time = ros::Time(0);
				while(1)
				{
					// 現在の map上の座標を取得
					now_tf = get_tf("base_link", "map", ros::Time::now());
					...
					person_current_time = ros::Time(0);
				...
				// tfの実行
				gm_output_pose = get_gm(gm_input_pose, "/map", "/base_link", ros::Time::now());
				...
				// min_obstacle_dist を元に戻す
				min_obstacle_set(i, 0);
