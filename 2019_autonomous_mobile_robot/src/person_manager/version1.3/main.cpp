...
void get_person_callback(const geometry_msgs::PointStamped::ConstPtr& sub_point)
{
	...
	if(28 < goal_index && goal_index < 41) // 20-28
	{
	...
	for(int i = start_point; i < goal_point.size(); i++) //i = 18 person
	{
		...
		while(ros::ok())
		{
			...
			else
			{
				...
				gm_base_pose.pose.position.x = now_person.position[0];
				gm_base_pose.pose.position.y = now_person.position[1];
				gm_base_pose.pose.position.z = 0.0;
				gm_base_pose.pose.orientation.x = 0.0;
				gm_base_pose.pose.orientation.y = 0.0;
				gm_base_pose.pose.orientation.z = 0.0;
				gm_base_pose.pose.orientation.w = 1.0;
				...
				while(1)
				{
					try
					{
						listener.lookupTransform("/map", "/base_link", person_time, transform);
					}
					catch (tf::TransformException &ex)
					{
						ROS_ERROR("%s",ex.what());
				...
				double map_dx = gm_map_pose.pose.position.x - transform.getOrigin().x();
				double map_dy = gm_map_pose.pose.position.y - transform.getOrigin().y();
